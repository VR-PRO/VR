...
var ejs = require('ejs');
var fs = require('fs');
var messageTpl = fs.readFileSync(__dirname + '/callback.ejs', 'utf-8');
...
var WxPay = {
    ...
    paysignjsapi: function(appid, attach, body, mch_id, nonce_str, notify_url, openid, out_trade_no, spbill_create_ip, total_fee, trade_type) {
        ...
        var string = this.raw(ret);
        string = string + '&key=' + config.key; //key为在微信商户平台(pay.weixin.qq.com)-->账户设置-->API安全-->密钥设置
        var crypto = require('crypto');
        var sign = crypto.createHash('md5').update(string, 'utf8').digest('hex');
        return sign.toUpperCase();
    },
    ...
    order: function(openid, bookingNo, _total_fee) {
        ...
        var notify_url = config.notify;
        var total_fee = Number(_total_fee) * 100;
    ...
    notify: function(obj) {
        var output = "";
        if (obj.body.xml.return_code == "SUCCESS") {
            var reply = {
                return_code: "SUCCESS",
                return_msg: "OK"
            };
            Order.updatePayResult(obj.body.xml.out_trade_no);
        } else {
            var reply = {
                return_code: "FAIL",
                return_msg: "FAIL"
